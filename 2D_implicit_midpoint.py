...
def D_tilde(d, u, s, h):
   return sqrt(D(d) + (h**2.5)*(((s - div(D(d)*grad(u)) - l(d))**2)**1.25))
...
L = (
(q*(u1 - u0) + Dt*(D_tilde(sin(2*pi*time) - b - uh, uh, sh, Dt)*inner(grad(uh),grad(q)) - q*l(sin(2*pi*time) - b - uh)))*dx(degree = 4) +
(p*(u1 - u0) - Dt*sh*p)*dx
)
